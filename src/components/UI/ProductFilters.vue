  ...
  <div class="content-wrapper">
    <div class="products-page">
      <div class="sidebar">
        <div class="filter-group">
          <div class="filter-header" @click="toggleFilter('Categories')">
            <h3>Категории</h3>
            <span class="arrow" :class="{ open: expandedFilters['Categories'] }">▼</span>
          </div>
        ...
        <div class="filter-group">
          <div class="filter-header" @click="toggleFilter('Brand')">
            <h3>Бренды</h3>
            <span class="arrow" :class="{ open: expandedFilters['Brand'] }">▼</span>
          </div>
          <transition name="expand">
        ...
        <div class="filter-group" v-for="(options, name) in characteristicOptions" :key="name">
          <div class="filter-header" @click="toggleFilter(name)">
            <h3>{{ name }}</h3>
            <span class="arrow" :class="{ open: expandedFilters[name] }">▼</span>
          </div>
          <transition name="expand">
      ...
      <div class="products-container">
        <div class="top-bar">
          <span>Selected Products: {{ filteredProducts.length }}</span>
        </div>
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter((product) => {
    ...
    const matchCategory = !selectedCategories.value.length || selectedCategories.value.includes(product.category);
    const matchChars = matchesCharacteristics(product);
    
    return matchSearch && matchBrand && matchCategory && matchChars;
  });
})
